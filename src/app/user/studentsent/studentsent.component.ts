...
@Component({
  selector: 'app-studentsent',
  templateUrl: './studentsent.component.html',
  styleUrl: './studentsent.component.css'
})
export class StudentsentComponent {
messages: any[] = [];
  userName: string = '';
  page = 0;
   pageSize = 3;
  ...
  constructor(private studentService: StudentService,private userAuthService: UserAuthService,private snackBar: MatSnackBar,private dialog: MatDialog) { }
  ngOnInit(): void {
  ...
  loadSentMessages(): void {
      this.studentService.getMessagesFromSender(this.userName, this.page, this.pageSize).subscribe(
        response => {
    ...
    if (confirmed) {
      ...
      this.studentService.deleteAllSentMessages(this.userName).subscribe({
        next: () => {
  ...
  deleteMessage(id: number): void {
      if (confirm('Delete this message?')) {
        this.studentService.deleteMessage(id).subscribe(() => this.loadSentMessages());
      }
    }
    markAllSentAsRead(): void {
    const userName = this.userAuthService.getUsername();
    this.studentService.markAllAsReadFromSender(this.userName).subscribe({
      next: (messages) => {
  ...
  markAsRead(msg: Message): void {
    this.studentService.markAsRead(msg.id!).subscribe(() => {
      msg.isRead = true;
    });
  }
