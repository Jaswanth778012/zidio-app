...
@Component({
  selector: 'app-studentmessage',
  templateUrl: './studentmessage.component.html',
  styleUrl: './studentmessage.component.css'
})
export class StudentmessageComponent implements OnInit {
 messages: any[] = [];
  ...
  loadInboxMessages(): void {
    this.studentService.getInboxMessages(this.userName, this.page, this.pageSize).subscribe(
      response => {
   ...
   goToPage(pageNumber: number): void {
    ...
    if (confirmed) {
      ...
      this.studentService.deleteAllInboxMessages(this.userName).subscribe({
        next: () => {
  ...
  deleteMessage(id: number): void {
      if (confirm('Delete this message?')) {
        this.studentService.deleteMessage(id).subscribe(() => this.loadInboxMessages());
      }
    }
    markAllInboxAsRead(): void {
    const userName = this.userAuthService.getUsername();
    this.studentService.markAllAsReadFromRecipient(this.userName).subscribe({
      next: (messages) => {
